...
export class Global {
  ...
  public static wbf: typeof ProcessService.prototype.writeBuffer = notInitializedFunc;
  ...
  public static createResolver = <T, U = {}>(
    baseOffset: any,
    offsetList: T,
    typesForSignatures?: { [index: string]: MemoryTypes },
    extendBy?: U
  ): Resolver<T> => {
    ...
    for (const k in offsetList as Object) {
      if (offsetList[k]) {
        resolver[k] = (type?: MemoryTypes) => Global.rpm(resolver.base + offsetList[k], type || typesForSignatures[k]);
